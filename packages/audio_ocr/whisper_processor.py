    ...
    def transcribe_video_audio(self, video_path: str) -> List[AudioSegment]:
        ...
        return [
            AudioSegment(
                start_s=0.0,
                end_s=5.2,
                text="Welcome to the advanced video search and indexing session.",
                confidence=0.98,
            ),
            AudioSegment(
                start_s=5.5,
                end_s=12.1,
                text="Today we discuss late interaction multi-vector embeddings with ColPali.",
                confidence=0.96,
            ),
            AudioSegment(
                start_s=12.4,
                end_s=18.8,
                text="Notice how token-level cross-attention highlights visual patches on the screen.",
                confidence=0.94,
            ),
        ]
