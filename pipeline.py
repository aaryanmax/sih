...
class ColPaliSearchPipeline:
    def __init__(
        ...
        # In dummy mode we skip loading real YOLO/OCR models too (no GPU/
        ...
        logger.info(
            ...
            mode, proj_dim, use_yolo, use_ocr,
        )
    ...
    def ingest_video(
        ...
        Args:
            ...
            frames: list of PIL frames (already extracted at ~2s intervals
                     upstream, matching the team's video_chunker.py convention).
        ...
        logger.info(
            ...
            video_id, len(frames), time.time() - t0,
        )
    ...
    def ingest_video_file(
        ...
        """
        Extracts frames and timestamps from a real .mp4 video file using OpenCV
        and indexes them end-to-end.

        Args:
            video_path: Path to .mp4 / .avi / .mov video file.
            video_id: Optional video identifier (defaults to filename without extension).
            interval_sec: Time interval in seconds between extracted frames (default 2.0s).
            save_thumbnails_dir: Optional directory to save frame thumbnails for video player.
        """
        import os
    ...
    def search(
        ...
        If the query mentions a COCO object (e.g. "red car", "dog"):
          - object_mode="filter": only frames where YOLO detected that
            object are searched at all (fastest, but misses frames YOLO
            failed to detect the object in).
          - object_mode="boost" (default): all frames are searched, but
            matching YOLO labels and OCR text add a score bonus — safer
            when detection isn't perfectly reliable.
          - object_mode="off": ignore YOLO/OCR signals entirely, ColPali
            MaxSim score only.
        ...
        # When filtering, we already know the exact candidate set from
        # YOLO labels, so we skip the FAISS ANN step entirely and run
        # exact MaxSim directly over just those frames (see
        # faiss_index.py's ColPaliFaissIndex.search_within for the exact
        # scoring reused here).
        if self.use_yolo and object_mode == "filter" and self._frame_object_labels:
            keywords = extract_object_keywords(query_text)
            if keywords:
                allowed_frames = {
                    fid for fid, labels in self._frame_object_labels.items()
                    if keywords.issubset(labels)
                }
                logger.info(
                    "Object pre-filter active: %d/%d frames match %s",
                    len(allowed_frames), len(self._frame_object_labels), keywords,
                )
        ...
        if use_gemini and results:
            results = rerank_results_with_gemini(query_text, results, api_key=gemini_api_key)

        elapsed_ms = (time.time() - t0) * 1000

        logger.info(
            "Search '%s' (object_mode=%s, use_gemini=%s) -> %d results in %.1fms",
            query_text, object_mode, use_gemini, len(results), elapsed_ms,
        )
        return results
