...
class ColPaliFaissIndex:
    def __init__(self, dim: int = 128, index_type: str = "hnsw", hnsw_m: int = 32):
        ...
        if index_type == "flat":
            ...
            # nlist=100 is a reasonable starting point; tune as corpus grows
        ...
        # Keeps every frame's full patch set around for the exact MaxSim
        # rerank step (step 3 above).
    ...
    def add_frame(self, frame_id: str, patch_vectors: np.ndarray):
        ...
        if patch_vectors.ndim != 2 or patch_vectors.shape[1] != self.dim:
            raise ValueError(
                f"Expected patch_vectors of shape (N, {self.dim}), got {patch_vectors.shape}"
            )
    ...
    def search(
        ...
        Args:
            ...
            candidate_pool: cap on how many distinct candidate frames get
                             exact-reranked (keeps stage 2 bounded even if
                             stage 1 surfaces a lot of distinct frames).
        ...
        logger.info(
            ...
            len(candidate_frame_ids), query_vectors.shape[0],
        )
    ...
    def search_within(
        ...
        """
        Exact MaxSim search restricted to a known candidate set (used when
        an upstream filter — e.g. YOLO object pre-filter — has already
        narrowed things down, so the FAISS ANN stage can be skipped
        entirely and we go straight to exact scoring).
        """
        query_vectors = query_vectors.astype(np.float32)
        scored = [
            (frame_id, maxsim_score(query_vectors, self._frame_patches[frame_id]))
    ...
    def save(self, path_prefix: str):
        """Persist the FAISS index + metadata to disk (two files:
        <prefix>.faiss and <prefix>.meta.pkl)."""
    ...
    def load(cls, path_prefix: str) -> "ColPaliFaissIndex":
        ...
        obj._index = faiss.read_index(f"{path_prefix}.faiss")
...
if __name__ == "__main__":
    ...
    # Simulate 50 keyframes, each with 64 patches (typical ColPali scale)
