...
class QueryEncoder:
    ...
    def _ensure_loaded(self):
        ...
        self._projection = nn.Linear(lm_hidden_size, self.proj_dim, bias=False).to(
...
def encode_query(text: str, _cache: dict = {}) -> np.ndarray:
    """
    Module-level convenience function (matches the import style used in
    apps/api/routes/search.py: `from embeddings.query_encoder import encode_query`).
    Keeps a single lazily-created QueryEncoder instance around so repeated
